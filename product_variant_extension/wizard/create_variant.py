    ...
    def default_get(self, fields):
        ...
        for template in product_template_ids:
            ...
            if not template.has_dynamic_attributes():
                ...
                # print('template.attribute_condition_ids', template.attribute_condition_ids)
                ...
                for line in template.attribute_condition_ids:
                    # print('line.attribute_value_ids.ids', line.attribute_value_ids.ids)
                    # print('line.value_id.id', line.value_id.id)
                    ...
                    if value_ids and value_ids not in existing_variants and template.attribute_condition_ids.ids:
                        for allowed_comb in allowed_combination:
                            ...
                            if set(allowed_comb).issubset(set(value_ids_list)):
                                ...
                                sq_no += 1
            # added_value = {}
            # for line in template.attribute_condition_ids.filtered(lambda val: not val.flg):
            #     update_variants = []
            #     for variant in template.product_variant_ids:
            ...
            #
            #     exist_attrib = line.value_id.attribute_id.id
            #     for variant in update_variants:
            #         if variant not in added_value:
            #             added_value[variant] = [line.value_id.id]
            #         else:
            #             added_value[variant].append(line.value_id.id)

            # for prd_rec in Product.browse(added_value.keys()):
            #     variants_to_create.append((0, 0, {
            #         'sq_no': sq_no,
            #         'add_flag': True,
            #         'attribute_value_ids': prd_rec.attribute_value_ids.ids + added_value[prd_rec.id],
            #     }))
            #     sq_no += 1
            #
            # no_update_variants = list(set(template.product_variant_ids.ids) - set(list(added_value.keys())))
            # print('added_value', added_value)
            #
            # if list(added_value.keys()):
            #     for prd_rec in Product.browse(no_update_variants):
            #         variants_to_create.append((0, 0, {
            #             'sq_no': sq_no,
            #             'add_flag': True,
            #             'attribute_value_ids': prd_rec.attribute_value_ids.ids,
            #         }))
            #         sq_no += 1
        ...
        result['new_variant_ids'] = explode_variants_to_create
    ...
    def create_product(self):
        ...
        for tmpl_id in product_template_ids:
            ...
            if not tmpl_id.has_dynamic_attributes():
                ...
                # newly_creating_explode = 0
                # if not variants_to_create:
                #     variants_to_create = tmpl_id.product_variant_ids
                #     newly_creating_explode = 1

                # print('variants_to_create', variants_to_create)
                # if any(val.explode_flag for val in tmpl_id.attribute_line_ids):
                #     print('inside----------------------------->')
                #     sq_no = 1
                #     length_new_variant = len(variants_to_create)
                #     print('length_new_variant', length_new_variant)
                #     explode_variants_to_create = []
                #
                #     for res in tmpl_id.attribute_line_ids.filtered(lambda val: val.explode_flag):
                #         dup_count = length_new_variant
                #         print('dup_count', dup_count)
                #         value_count = len(res.value_ids)
                #         print('value_count', value_count)
                ...
                #                     if newly_creating_explode == 0:
                #                         # exist_att = each[2]['attribute_value_ids'][:]
                #                         exist_att = each['attribute_value_ids'][0][2][:]
                #                     else:
                #                         exist_att = each.attribute_value_ids.ids[:]
                #                     exist_att.append(value_ids[count])
                #                     print('value_ids[count]', value_ids[count])
                #                     input(exist_att)
                #                     explode_variants_to_create.append({
                #                     'product_tmpl_id': tmpl_id.id,
                #                     'attribute_value_ids': [(6, 0, list(exist_att))],
                #                     'active': tmpl_id.active,
                #                 })
                #                     print('explode_variants_to_create', explode_variants_to_create)
